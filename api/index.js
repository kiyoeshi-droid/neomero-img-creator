...
// 注意: Vercelのサーバーレス環境ではファイルシステムへの書き込みができないため、
// 読み取り専用で動作します。書き込みが必要な場合は外部ストレージ（Vercel KV、MongoDB等）を使用してください。
const PROMPTS_PATH = process.env.VERCEL 
  ? path.join(process.cwd(), 'prompts.json')
  : path.join(__dirname, '..', 'prompts.json');

// Vercel環境での書き込みを無効化（読み取り専用モード）
...
app.put('/api/characters/:id', async (req, res) => {
  if (IS_READ_ONLY) {
    return res.status(503).json({ error: 'Write operations are not available in read-only mode. Please use a local server for write operations.' });
  }
